...
public class Game {
	...
	public void move(Direction d) throws NotEnoughResourcesException, UnallowedMovementException {
		if (hasEffect(getCurrentChampion(), "Root"))
			throw new UnallowedMovementException("You can not move while\n being rooted");
		if (getCurrentChampion().getCurrentActionPoints() < 1)
			throw new NotEnoughResourcesException("You need at least one\n action point to move");
	...
	public void attack(Direction d)
		...
		if (getCurrentChampion().getCurrentActionPoints() < 2)
			throw new NotEnoughResourcesException("You need at least two action point\n to perform a normal attack");
		int currx = (int) getCurrentChampion().getLocation().getX();
		int curry = (int) getCurrentChampion().getLocation().getY();
	...
	InvalidTargetException, CloneNotSupportedException {
		validateCastAbility(a);
		if (board[x][y] == null)
			throw new InvalidTargetException("You can not cast an ability\n on an empty cell");
		int distance = Math.abs((int) getCurrentChampion().getLocation().getX() - x)
				+ Math.abs((int) getCurrentChampion().getLocation().getY() - y);
		if (distance > a.getCastRange())
		...
		if (board[x][y] instanceof Champion) {
			...
			if (friendly && a instanceof DamagingAbility)
				throw new InvalidTargetException("Can not cast damaging ability\n on friendly targets");
...
public void endTurn(){
			...
			if(getCurrentChampion().getAppliedEffects().get(i) instanceof Stun){
				...
				for(int k=0 ; k<getCurrentChampion().getAppliedEffects().size() ; k++){
					...
					if(getCurrentChampion().getAppliedEffects().get(k).getDuration()==0) {
						Effect temp = getCurrentChampion().getAppliedEffects().get(k);
						getCurrentChampion().getAppliedEffects().remove(k);
						temp.remove(getCurrentChampion());
		...
		for(int i=0 ; i<getCurrentChampion().getAppliedEffects().size() ; i++){
			...
			if(getCurrentChampion().getAppliedEffects().get(i).getDuration()==0) {
				Effect temp = getCurrentChampion().getAppliedEffects().get(i);
